...
| 板端 C++ 预处理 | 平均 7.601 ms；P50/P90/P95 = 7.116/10.069/10.787 ms |
| 板端 C++ NPU | 平均 9.602 ms；P50/P90/P95 = 9.608/9.654/9.667 ms |
| 板端 C++ 后处理 | 平均 16.223 ms；P50/P90/P95 = 16.117/17.230/17.677 ms |
| 板端 C++ 稳态端到端 | 平均 33.566 ms（约 29.79 FPS）；P50/P90/P95 = 33.042/36.488/37.782 ms |
...
一次性模型加载、最终 JSON 序列化及 COCOeval。正式运行 ID 为 `20260908_cpp_full_v2`，
逐图 CSV 和汇总 JSON 位于 89 的
`examples/output/board_cpp_accuracy/20260908_cpp_full_v2/`。峰值内存仍缺少可信证据，继续保留
为待补测。

当前交付状态为“板端已验证”。峰值内存补齐并保存可追溯日志后，才能升级为“完整交付”。
