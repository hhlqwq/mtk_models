...
test -f "${SCRIPT_DIR}/inference_demo/compare_results.py"
...
readonly EXISTING_SHA256_LINE="$(ssh "${SSH_OPTIONS[@]}" "${BOARD_HOST}" \
    "if test -f '${BOARD_MODEL_DIR}/model_fp32_opset13.onnx'; then \
        sha256sum '${BOARD_MODEL_DIR}/model_fp32_opset13.onnx'; fi")"
...
scp "${SSH_OPTIONS[@]}" \
    ...
    "${SCRIPT_DIR}/inference_demo/compare_results.py" \
    "${BOARD_HOST}:${BOARD_MODEL_DIR}/"
