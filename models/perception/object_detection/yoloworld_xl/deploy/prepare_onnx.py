...
def create_raw_head_model(source_path: Path, output_path: Path) -> onnx.ModelProto:
    """裁掉 DFL Softmax/MatMul,输出类别 logits 与 64 通道框 logits."""
    source = onnx.load(source_path)
    class_outputs = [source.graph.output[index].name for index in (0, 2, 4)]
    output_names = []
    for class_output, bbox_output in zip(
        class_outputs, RAW_BBOX_OUTPUTS, strict=True
    ):
        output_names.extend((class_output, bbox_output))
    ...
    raw_model = onnx.load(output_path)
    output_shapes = [tensor_shape(item) for item in raw_model.graph.output]
    if output_shapes != RAW_OUTPUT_SHAPES:
        raise ValueError(f"Raw 输出形状不匹配: {output_shapes}.")
    if any(node.op_type == "Softmax" for node in raw_model.graph.node):
        raise ValueError("Raw 模型仍包含 Softmax.")
    return raw_model
